...
def align_gravity_camera(clusters_A, clusters_B, cam_A: CameraEvidence,
    ...
    def _pair_metrics(pi, s, R, t):
        ...
        per_pair_res = np.linalg.norm(transform_points(srcs, s, R, t) - dsts, axis=1)
        res = float(np.mean(per_pair_res))
        max_res = float(np.max(per_pair_res))
        ...
        return res, max_res, size_err, norm_err
    ...
    def _add_multi(pi):
        ...
        res, max_res, size_err, norm_err = _pair_metrics(pi, s, R, t)
        cand.append({"pi": pi, "s": s, "R": R, "t": t, "res": res, "max_res": max_res,
    ...
    #
    # Grouped by CATEGORY first (not run once over ALL of A/B mixed together) -
    # confirmed a real bug on real data (`server`): A had both windows and doors,
    # B had only doors. Every full-length combination (`len(A)` or `len(B)` elements)
    # necessarily included at least one window once doors ran out, which
    # `_add_multi`'s category check then rejected - so EVERY multi-opening
    # candidate silently failed, leaving only the single-opening path (whose
    # 1-correspondence fit is trivially "perfect" and inherently ambiguous,
    # even though a clean 2-door correspondence existed and would have won
    # easily on its own `geom` score if it had ever been proposed as a
    # candidate at all). Grouping by category first means a real subset like
    # "match A's 4 doors onto 4 of B's 5 doors" gets tried on its own,
    # unblocked by A's windows having nothing to match in B.
    #
    # ALSO tries every subset SIZE from 2 up to the category's own
    # min(len_a, len_b), not just the single largest size - confirmed a
    # 2nd real bug on the same `server` data: even after the category fix
    # above, forcing "match ALL 4 of A's doors" (there being 4 vs B's 5)
    # still forced 2 doors from a genuinely DIFFERENT, unrelated physical
    # wall into the SAME correspondence set as the true matching pair,
    # because no smaller (e.g. 2-door) same-wall-only subset was ever
    # tried on its own. A real building can easily have more doors on one
    # side that simply have no counterpart at all (a door to a different,
    # unrelated room) - the correspondence search must allow using fewer
    # than every available opening.
    if n_min >= 2:
        for category in set(o.category for o in A) & set(o.category for o in B):
            idx_a = [i for i, o in enumerate(A) if o.category == category]
            idx_b = [j for j, o in enumerate(B) if o.category == category]
            max_k = min(len(idx_a), len(idx_b))
            if max_k < 2:
                continue
            for k in range(2, max_k + 1):
                for sub_a in itertools.combinations(idx_a, k):
                    for comb_b in itertools.permutations(idx_b, k):
                        _add_multi(list(zip(sub_a, comb_b)))
    ...
    # max_res gate (not just the MEAN res folded into `geom`): every
    # individual pair in the candidate must itself be within one
    # T_center radius - confirmed a real bug on real data (`server`):
    # ranking by match count (needed to fix a separate real bug on
    # `chua_thay` - see below) let a 3-opening candidate that quietly
    # mixed in 1 pair from a totally different, unrelated wall win over
    # the correct 2-opening one, because the aggregate/mean-based checks
    # here didn't reject it (2 excellent pairs pulled the MEAN residual
    # down enough to still pass) even though that one bad pair's own
    # residual was ~5x T_center - obviously not from the same wall.
    valid = [c for c in keep if c["side"] > 0 and c["size_err"] <= SIZE_TOL
             and c["norm_err"] <= NORM_TOL and c["dom"] >= 0.5
             and c["max_res"] <= T_center]
    # Rank by MATCH COUNT first, `geom` (mean residual + size/normal error)
    # only as the tie-break within that count - not `geom` alone.
    # Confirmed a real bug on real data (`chua_thay`) once the search
    # above started trying same-category SUBSET sizes too (needed to fix
    # a different real bug on `server` - see that fix's note): a small
    # subset of an otherwise-consistent 5-opening match can have a LOWER
    # *mean* residual than the full 5-opening match purely by chance
    # (leaving out whichever 1-2 openings fit slightly worse pulls the
    # average down), which would make `geom`-only ranking prefer throwing
    # away real, consistent evidence. Ranking by count first restores
    # "use all the consistent evidence you have" while still correctly
    # letting a smaller-but-genuinely-different-wall candidate win when
    # the larger one is actually wrong (that case fails the `side`/
    # `size_err`/`norm_err` gates above and never reaches `valid` at all,
    # as confirmed for `server`'s wrong 4-opening mixed-wall candidate).
    valid.sort(key=lambda c: (len(c["pi"]), -c["geom"]), reverse=True)

    if valid:
        top = valid[0]
        top_n = len(top["pi"])
        status, reason = "CONFIDENT", "OK (gravity-locked + size + camera-side)"
        # "near-tied" only means something when comparing 2 candidates of
        # the SAME size (2 distinct correspondences equally well-
        # supported) - a smaller subset of the winning candidate is not a
        # competing alternative, just less evidence.
        same_size_rivals = [c for c in valid[1:] if len(c["pi"]) == top_n]
        if same_size_rivals and (same_size_rivals[0]["geom"] - top["geom"]) < 0.25:
            status = "AMBIGUOUS"
