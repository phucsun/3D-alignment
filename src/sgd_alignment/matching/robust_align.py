...
def match_openings(clusters_A, clusters_B, tau_scene=0.18, size_floor_frac=0.5,
    ...
    hyps = _triangle_hypotheses(A, B, tau_scene, T_center)
    if not hyps:
        hyps = _two_opening_hypotheses(A, B)
    if not hyps:
        return AlignResult(1.0, np.eye(3), np.zeros(3), [], "NO_SOLUTION",
    ...
    if n_in < 3:
        ...
        for (s2, R2, t2) in hyps:
            ...
            be.append((len(m2), s2, R2, t2, m2))
        be.sort(key=lambda z: z[0], reverse=True)
...
def openings_from_manual_segmentation(path, window_keys=("cua_so",),
    ...
    for name in v.dtype.names:
        low = name.lower()
        if not low.startswith("scalar") or "cua" not in low:
            continue
        if "cloud" in low or "index" in low or "original" in low:
            continue
